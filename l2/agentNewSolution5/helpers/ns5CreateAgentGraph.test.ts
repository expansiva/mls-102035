/// <mls fileReference="_102035_/l2/agentNewSolution5/helpers/ns5CreateAgentGraph.test.ts" enhancement="_blank"/>
...
const ENTRY = path.join(NS_ROOT, 'agentNewSolution5.ts');
...
void test('createAgent static graph has no monaco/lit/widgets/DOM host deps', () => {
  ...
  assert.ok(rels.includes('agentNewSolution5.ts'), 'entry must be in the graph');
...
void test('guard goes red on a static widgets import (mutation of the detector)', () => {
  const poisoned = [
    'import { Widget } from "/_102035_/l2/agentNewSolution5/widgets/clarification.js";',
    'import { addMessage } from "/_102025_/l2/collabMessagesHelper.js";',
    'export function beforeNs5ModulePromptStep() { return window.location; }',
  ].join('\n');
