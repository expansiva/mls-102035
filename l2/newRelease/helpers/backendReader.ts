...
export function reviewArtifactFile(project: number, moduleName: string, name: 'backend' | 'effort'): Ns5FileInfo {
  return {
    project,
    level: 4,
...
export async function readReviewArtifact(project: number, moduleName: string, name: 'backend' | 'effort'): Promise<ReviewArtifactRead> {
  const file = reviewArtifactFile(project, moduleName, name);
