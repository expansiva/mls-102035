...
export class NewReleaseWorkflows102035 extends StateLitElement implements NewReleaseEditableTab<Ns5WorkflowsArtifact | null> {
  ...
  private currentArtifact(): Ns5WorkflowsArtifact | null {
    ...
    return isEditableWorkflowSchema(view.schema) ? view.raw as Ns5WorkflowsArtifact : null;
  }
  ...
  private taskIcon(kind: WorkflowTaskView['kind']) {
    ...
    if (kind === 'alert') return svg`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 17V11a6 6 0 0 1 12 0v6M4 17h16M10 20h4"/></svg>`;
  ...
  /** ns5_48: derived, never written — "Recurring duty · <schedule> · <actor>". */
  private recurringDutyLabel(process: WorkflowProcessView): string {
    const schedule = process.trigger?.schedule?.trim() || this.t('workflows.notRecorded');
    const actor = process.tasks.find(task => task.actorRef)?.actorRef || this.t('workflows.notRecorded');
    return `${this.t('workflows.recurringDuty')} · ${schedule} · ${actor}`;
  }
  ...
  render() {
    ...
    return html`<section class="nr-workflows"><header class="nr-workflows__hero"><div><span>${this.t('workflows.eyebrow')}</span><h2>${this.t('workflows.title')}</h2><p>${this.t('workflows.description')}</p><small class=${`schema-${view.schema}`}>${this.t(`workflows.schema.${view.schema}`)}</small></div><dl><div><dt>${this.t('workflows.processes')}</dt><dd>${view.processes.length}</dd></div><div><dt>${this.t('workflows.tasks')}</dt><dd>${workflowTaskCount(view.processes)}</dd></div><div><dt>${this.t('workflows.decisions')}</dt><dd>${view.journeyDecisions.length}</dd></div></dl></header><div class="nr-workflows__workspace"><aside class="nr-workflows__catalog"><header><span>${this.t('workflows.catalog')}</span>${isEditableWorkflowSchema(view.schema) ? html`<button type="button" @click=${() => this.addProcess()}>+ ${this.t('workflows.addProcess')}</button>` : nothing}</header>${view.processes.map(item => html`<button type="button" class=${item.processId === this.selectedProcessId ? 'is-active' : ''} @click=${() => { this.selectedProcessId = item.processId; }}><i>${this.taskIcon(item.tasks[0]?.kind || 'wait')}</i><span><strong>${item.title}</strong><small>${isRecurringDuty(item) ? this.recurringDutyLabel(item) : this.triggerSummary(item)}</small></span><b>${item.tasks.length}</b></button>`)}${!view.processes.length ? html`<p>${this.t('workflows.noProcesses')}</p>` : nothing}</aside>${process ? this.renderProcess(process) : html`<section class="nr-workflows__empty-detail"><h3>${this.t('workflows.noSelection')}</h3><p>${this.t('workflows.noSelectionDescription')}</p>${isEditableWorkflowSchema(view.schema) ? html`<button type="button" @click=${() => this.addProcess()}>+ ${this.t('workflows.addProcess')}</button>` : nothing}</section>`}</div>${this.renderDecisions()}${this.mode === 'edit' ? html`<footer class="nr-workflows__edit-footer"><span>${this.dirty ? this.t('workflows.unsaved') : this.t('workflows.noChanges')}</span><div><button type="button" @click=${() => this.cancelEdit()}>${this.t('workflows.cancel')}</button><button class="is-primary" type="button" ?disabled=${!this.dirty} @click=${() => void this.saveEdit()}>${this.t('workflows.save')}</button></div></footer>` : nothing}</section>`;
  }
}
